/*!
JS for Formloom 3
Copyright (c) 2014 Yabdab Inc.
http://yabdab.com/plugins/formloom/
Published: 2017-06-16 18:49:01 +0000
FormLoom v. 3.0.10 (141)
*/
...
/* Updated to 1.5.0 in version 3.0.3 -MY */
 ...
 * http://jqueryvalidation.org/category/methods/ <- explains the rules
 *
...
$.extend($.fn, {
	...
	rules: function( command, argument ) {
        ...
        // Add this to trigger iframe height
...
var form_selector = '#page7-ydfl-form';
var submitBtnText = $('#page7-ydfl-form button.ydfl-submit').html();
var loginBtnText = 'Login';
    
/* Validate and Submit
========================================================*/

$('#page7-ydfl-form').validate({
...
submitHandler: function(form) {
	...
	$(form).ajaxSubmit({
		beforeSubmit:function(){ 
				$(form_selector + ' .form-results').removeClass('alert-success').removeClass('alert-error');
				$('button[type=submit]').html('<i class="fa fa-circle-o-notch fa-spin fa-3x" style="font-size:18px;"></i>').prop('disabled', 'disabled');
		},
		error:function(){
				$(form_selector + ' .form-results').html('An Error Occured').show();
                $(window).trigger('send-height');
		},
        data: { url: parent },
        success:function(data){
				...
				if(rs.result == 'fail' || rs.result == 'loginfail')
				{
					...
					if(rs.redirect && rs.result != 'loginfail')
					{
						...
						var msg = '<div class="notification alert-error spacer-t10"><p><i class="fa fa-bomb"></i> <b>Oops! An error occurred.</b> ' + rs.msg + '</p><a href="#" class="close-btn">&times;</a></div>';
                        var btnText = (rs.result == 'loginfail') ? loginBtnText : submitBtnText;
						$('button[type=submit]').html(btnText).prop('disabled', false);
						$(form_selector + ' .form-results').html(msg).show();
					}
				...
				}else if(rs.result == 'loginsuccess')
				{
					...
					if(rs.redirect)
					{
						...
						var msg = '<div class="notification alert-success spacer-t10"><p><i class="fa fa-check"></i> <b>Your form was sent successfully.</b></p><a href="#" class="close-btn">&times;</a></div>';
						$('button[type=submit]').html('<i class="fa fa-check fa-3x" style="font-size:18px;"></i>').prop('disabled', false);
						setTimeout(function(){$('button[type=submit]').html(submitBtnText);}, 2000);
...
$( "input[name='form[item_19]']" ).rules( "add", {
required: true,
messages: {
} }); //  end add rule 
...
function reloadCaptcha(selector){
    var url = $(selector + " .ydfl-captcha").attr("src");
    var src = url.split('?r=');
    $(selector + " .ydfl-captcha").attr("src", src[0] + "?r=" + Math.random());
}
